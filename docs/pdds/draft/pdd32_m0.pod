...
thought of more accurately as a "runcore" object.  It contains the currently
...
The context objects are the actual execution units of C<M0>.  It contains
...
optional context to return to implicitly if it reaches the end of available ops
in the code block.
...
This layout also has the implication that the code execution is quite
...
Since all PMCs are just chunks of memory available for manipulation, structs
are trivial to implement.  Structs can be accessed and manipulated just like C
would on a traditional machine.  Therefore, structs are an implied construct,
and not enforced.
